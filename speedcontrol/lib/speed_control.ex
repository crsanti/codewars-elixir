defmodule SpeedControl do
  ...
  # First attempt
  # def get_hourly_distance(distance, time), do: 3600 * distance / time
  # def gps(_, sections) when length(sections) <= 1, do: 0

  # def gps(time, sections) do
  #   sections
  #   |> Enum.with_index()
  #   |> Enum.map(fn
  #     {val, 0} -> val
  #     {val, index} -> get_hourly_distance(val - Enum.at(sections, index - 1), time)
  #   end)
  #   |> Enum.max()
  #   |> round
  # end
